...
const updateTask = async (req, res) => {
    ...
    const task = await Task.findByIdAndUpdate({_id:taskId, createdBy:userId}, req.body, {new:true, runValidators:true, context: 'query'})
    if(!task){
...
const updateUser = async (req, res) => {
    ...
    if (req.user.id === id) {
        throw new BadRequestError('Admin cannot remove his own role as admin...but he can kinda do it manually if he has access to the db... so is this function useless?')
      }
   // Check for valid mongoose objectID
    if (!Types.ObjectId.isValid(id)) {
        throw new UnauthenticatedError('Invalid authentication')
      }
    const user = await User.findByIdAndUpdate({_id:userId}, req.body,  { new:true, runValidators: true, context: 'query' })
    if(!user){
